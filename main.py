...
# ---------------------------------------------------------
# SERVIDOR WEB PARA HEALTH CHECK DE RENDER
# ---------------------------------------------------------
class HealthCheckHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format, *args):
        return  # Ocultar logs HTTP repetitivos

def run_health_check_server():
    port = int(os.environ.get("PORT", 8080))
    server = HTTPServer(('0.0.0.0', port), HealthCheckHandler)
    logging.info(f"🌐 Servidor Web HTTP escuchando en el puerto {port}")
...
def generate_chart_url(history):
    ...
    chart_config = {
        ...
        "options": {
            "title": {"display": True, "text": "Histórico Binance P2P BDV"},
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🤖 *Bot Binance P2P con IA*\n\n"
        "• `/status` : Precios actualizados y spread.\n"
        "• `/grafico` : Histórico dinámico desde la BD.\n"
        "• `/prediccion` : Análisis de tendencia.\n"
        "• `/preguntar` : Hazle una pregunta a la IA.",
        parse_mode="Markdown"
    )
...
async def prediccion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(history) < 3:
        await update.message.reply_text("⏳ Esperando más registros para realizar la predicción...")
        return

    msg_wait = await update.message.reply_text("🧠 Analizando tendencia...")
...
async def grafico(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(history) < 2:
        await update.message.reply_text("⏳ Datos insuficientes en BD. Intenta en breve.")
        return
...
async def preguntar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Consulta a la IA vía API REST de Google Gemini."""
    if not GEMINI_API_KEY:
        await update.message.reply_text("⚠️ La variable GEMINI_API_KEY no está configurada en Render.")
        return

    if not context.args:
        await update.message.reply_text("❓ *Uso:* `/preguntar ¿Qué es el arbitraje P2P?`", parse_mode="Markdown")
        return
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            await msg_wait.edit_text(texto_respuesta)
        else:
            await msg_wait.edit_text(f"⚠️ Error al consultar IA (Código {response.status_code}).")
    except Exception as e:
        logging.error(f"Error Gemini: {e}")
        await msg_wait.edit_text("❌ Error de conexión al consultar la IA.")
        ...
        if spread_ves >= 10.0:
            for chat_id in USER_CHAT_IDS:
                try:
                    ...
                    logging.error(f"Error alerta {chat_id}: {e}")
...
def main():
    ...
    # Iniciar servidor HTTP en segundo plano para Render Health Check
    ...
    print("🚀 Bot e interfaz HTTP iniciados correctamente...")
